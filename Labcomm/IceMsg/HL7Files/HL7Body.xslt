...
PID|<xsl:value-of select="PID.1"/><xsl:text>||</xsl:text>
	...
	<xsl:value-of select="PID.11/XAD.1/SAD"/>^<xsl:value-of select="PID.11/XAD.2"/>^<xsl:value-of select="PID.11/XAD.3"/><xsl:text>|</xsl:text>
	<xsl:apply-templates select="NTE"/> 
</xsl:template>
...
<xsl:value-of select="PV1.3/PL.4/HD.1"/><xsl:text>|</xsl:text>
